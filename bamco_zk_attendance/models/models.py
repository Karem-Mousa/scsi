...
class zk_attendance_tmp(models.Model):
    ...
    def process_data(self):
        ...
        # records = self.search([]).write({'logged': False})
        ...
        for emp in employees:
            emp_obj = self.env['hr.employee'].search([('attendance_id_char', '=', emp)], limit=1)
            if not emp_obj:
                continue
            datetime_list = records.filtered(
                lambda x: x.user_number == emp).mapped('date')
            dates = [x.date().strftime("%Y-%m-%d") for x in datetime_list]
            # dates = self.get_range_of_dates('2023-01-01', '')
            ...
            for date in dates_unique:
                ...
                # create atrendance record with check in=check out
                    ...
                    date_out = max(date_ins.mapped('date')) - time.timedelta(hours=1)
                    date_in = min(date_ins.mapped('date')) - time.timedelta(hours=1)
