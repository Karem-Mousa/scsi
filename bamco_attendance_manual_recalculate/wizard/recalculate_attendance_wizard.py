...
class recalculate_attendance_wizard(models.TransientModel):
    ...
    def delete_last_days_attendance(self, starting_date, end_date):
        # delete attendance
        starting_datetime = starting_date + ' 00:00:00'
        end_datetime = end_date + ' 23:59:00'
        self.env['hr.attendance'].search([('check_in', '>=', starting_datetime),
                                          ('check_out', '<=', end_datetime)]).unlink()
    ...
    def recalculate_data(self):
        if self.recalculate_attendance:
            ...
            self.delete_last_days_attendance(str(self.start_date), str(self.end_date))
            # recalculate again
