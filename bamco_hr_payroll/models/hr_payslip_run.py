...
class HrPayslipRun(models.Model):
    ...
    def employee_fields(self):
        ...
        employee_fields = {
            'name': ('0', 'Name'),
            'identification_id': ('0', 'رقم الهوية'),
            'attendance_id_char': ('0', 'الكود'),
            'area.name': ('0', 'إدارة'),
            'branch.name': ('0', 'الفرع'),
            'gender': ('1', 'Gender')
        }
    ...
    def _fetch_data(self):
        ...
        for payslip in self.slip_ids:
            ...
            for idx, field in enumerate(employee_fields):
                ...
                for name in names:
                    object = getattr(object, name, '0')

                value = object if object else '0'
    ...
    def generate_excel(self):
        ...
        return {
            ...
            'name': 'payslips',
            'url': '/web/content/hr.payslip.run/%s/gentextfile/payslips.xlsx?download=true' % (
                self.id),
            'target': 'new'
        }
